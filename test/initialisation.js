...
test('TinyColor initialization', t => {
	...
	const object = {h: 180, s: 0.5, l: 0.5}
	const color = tinycolor(object)
	t.true(object.s === 0.5, 'when given an object, the original object is not modified…')
...
test('Original input', t => {
	...
	const tinycolorObject = tinycolor(colorRgbMix)
	const inputObject = {r: 100, g: 100, b: 100}
	...
	t.true(tinycolor(tinycolorObject).getOriginalInput() === colorRgbMix, 'when given a tinycolor instance, the color string is returned')
	t.true(tinycolor(inputObject).getOriginalInput() === inputObject, 'when given an object, the object is returned')
