...
const isOnePointZero = n => typeof n === 'string' && n.indexOf('.') !== -1 && parseFloat(n) === 1; // Check to see if string passed in is a percentage

const isPercentage = n => typeof n === 'string' && n.indexOf('%') !== -1; // Don't let the range of [0,255] come back in [0,1].
...
const hasAlpha = rgba => rgba.a < 1 && rgba.a >= 0; // Force a number between 0 and 1

const clamp01 = val => mathMin(1, mathMax(0, val)); // Force a hex value to have 2 characters

const pad2 = c => c.length === 1 ? `0${c}` : `${c}`; // <http://www.w3.org/TR/css3-values/#integers>
...
const CSS_UNIT = `(?:${CSS_NUMBER})|(?:${CSS_INTEGER})`; // `isValidCSSUnit`
// Take in a single string / number and check to see if it looks like a CSS unit

const isValidCSSUnit = color => new RegExp(CSS_UNIT).test(color); // `isValidCSSUnitRGB`
// Take in a rgb object check to see if it looks like a CSS unit

const isValidCSSUnitRGB = rgb => isValidCSSUnit(rgb.r) && isValidCSSUnit(rgb.g) && isValidCSSUnit(rgb.b); // Actual matching.
// Parentheses and commas are optional, but not required.
// Whitespace can take the place of commas or opening paren

const PERMISSIVE_MATCH3 = `[\\s|\\(]+(${CSS_UNIT})[,|\\s]+(${CSS_UNIT})[,|\\s]+(${CSS_UNIT})\\s*\\)?`;
const PERMISSIVE_MATCH4 = `[\\s|\\(]+(${CSS_UNIT})[,|\\s]+(${CSS_UNIT})[,|\\s]+(${CSS_UNIT})[,|\\s]+(${CSS_UNIT})\\s*\\)?`; // Take input from [0, n] and return it as [0, 1]
...
const rgbaToArray = rgba => rgba.a === 1 ? [rgba.r, rgba.g, rgba.b] : [rgba.r, rgba.g, rgba.b, mathRound(rgba.a * 255)]; // `rgbaToHex`
...
class TinyColor {
  ...
  constructor(color, opts = {}) {
    color = color || ''; // If input is already a tinycolor, return itself
...
const isValidCSSUnitHSL = hsl => isValidCSSUnit(hsl.h) && isValidCSSUnit(hsl.s) && isValidCSSUnit(hsl.l); // `rgbToHsl`, `hslToRgb` modified from:
...
const isValidCSSUnitHSV = hsv => isValidCSSUnit(hsv.h) && isValidCSSUnit(hsv.s) && isValidCSSUnit(hsv.v); // `rgbToHsv`, `hsvToRgb` modified from:
// <http://mjijackson.com/2008/02/rgb-to-hsl-and-rgb-to-hsv-color-model-conversion-algorithms-in-javascript>
// `rgbToHsv`
